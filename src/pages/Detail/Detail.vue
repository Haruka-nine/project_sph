        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" :class="{active: spuSaleAttrValue.isChecked==='1'}"
                    ...
                    @click="changeActive(spuSaleAttrValue,spuSaleAttr)">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!--以前我们的路由跳转就是从A路由跳转到B路由，这里在加入购物车，进行路由跳转之前，发请求，把你要购买的产品信息通过请求的方式告诉服务器-->
  ...
  export default {
    ...
    methods:{
      changeActive(saleAttrValue,saleAttr){
        //这里随便写的，看个笑话就行，毕竟后边也不给服务器传数据，选了也白选
        //个人认为这里后端是有问题的，应该每个版本和每个搭配都做出一个对象，有独立的id
        //遍历
      ...
      addShopCar(){
        ...
        this.$store.dispatch('detail/addOrUpdateShopCart', {skuId:this.$route.params.skuId, skuNum:this.skuNum}).then(()=>{
          //进行路由跳转的时候需要将产品的信息带给下一级的组件
          //其实可以全部都用query参数传过去，但建议复杂参数使用会话存储,会话结束数据就消失
          //本地存储||会话存储一般存储字符串
